...
class SeparableConv2D(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, bias=False):
        ...
        self.depthwise = spectral_norm(nn.Conv2d(in_channels, in_channels, kernel_size=3,
                                                 stride=stride, padding=1, groups=in_channels, bias=bias))
        self.pointwise = spectral_norm(nn.Conv2d(in_channels, out_channels,
...
class ConvBlock(nn.Module):
    def __init__(self, channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False):
        ...
        self.conv = spectral_norm(nn.Conv2d(channels, out_channels,
...
class InvertedResBlock(nn.Module):
    def __init__(self, channels=256, out_channels=256, expand_ratio=2, bias=False):
        ...
        self.depthwise_conv = spectral_norm(nn.Conv2d(bottleneck_dim, bottleneck_dim,
                                                      kernel_size=3, groups=bottleneck_dim, stride=1, padding=1, bias=bias))
        self.conv = spectral_norm(nn.Conv2d(bottleneck_dim, out_channels,
                                            kernel_size=1, stride=1, bias=bias))
